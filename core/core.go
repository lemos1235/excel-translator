...
// 为了兼容性保留以下函数
func translateCells(tasks []TranslationTask, cfg *config.Config, callback func(original, translated string)) []TranslationResult {
	// 初始化 OpenAI 客户端
	openaiClient := openai.NewClient(
		option.WithBaseURL(cfg.LLM.APIURL),
		option.WithAPIKey(cfg.LLM.APIKey),
	...
	client := &openaiClient
	return translateCellsWithClient(tasks, client, cfg, callback)
}

// 为了兼容性保留以下函数
func TranslateText(textToTranslate string, cfg *config.Config) (string, error) {
	// 初始化 OpenAI 客户端
	openaiClient := openai.NewClient(
		option.WithBaseURL(cfg.LLM.APIURL),
		option.WithAPIKey(cfg.LLM.APIKey),
	...
	client := &openaiClient
	return TranslateTextWithClient(client, textToTranslate, cfg)
}
